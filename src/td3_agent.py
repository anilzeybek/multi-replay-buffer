...
class TD3Agent:
    ...
    def save(self):
        os.makedirs(f"saved_networks/{self.env_name}", exist_ok=True)
        torch.save({"actor": self.actor.state_dict(),
                    "critic": self.critic.state_dict(),
                    "t": self.t
                    }, f"saved_networks/{self.env_name}/actor_critic.pt")

        self.rb.save_transitions(f"saved_networks/{self.env_name}/rb.npz")

    def load(self):
        checkpoint = torch.load(f"saved_networks/{self.env_name}/actor_critic.pt")
        ...
        self.rb.load_transitions(f"saved_networks/{self.env_name}/rb.npz")
